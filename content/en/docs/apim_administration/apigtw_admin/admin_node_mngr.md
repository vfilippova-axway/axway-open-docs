...
{{< alert title="Note" color="primary" >}}When external CA signs the certificate, its extensions, such as Key Usage, must be copied from the CSR request.

Subject Distinguished Name (DN) of the certificate after signing must be the same as in CSR and relative DNs must be preserved.{{< /alert >}}
