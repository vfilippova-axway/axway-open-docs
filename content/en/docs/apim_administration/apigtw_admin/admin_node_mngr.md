...
The Subject distinguished name (DN) of the certificate after signing must also be the same as in the CSR and relative DNs must be preserved.
You won't be able to submit the certificate to `managedomain` that has different extensions or subject so it is important to check with the CA in advance to make sure these requirements are met.
{{< /alert >}}
