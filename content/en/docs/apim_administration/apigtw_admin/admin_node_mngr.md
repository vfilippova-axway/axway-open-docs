...
When external CA signs the certificate, all its key usage and extended key usage extensions, must be copied from the CSR request.

Subject distinguished name (DN) of the certificate after signing must be the same as in CSR and relative DNs must be preserved.
You won't be able to submit the certificate to `managedomain` that has different extensions or subject.
{{< /alert >}}
